...
const byId = (state = {}, { type, payload }) => {
  switch (type) {
    ...
    case types.COMMENTS_ADD:
      ...
      return _extends({}, state, { [payload.id]: _extends({}, payload.comment) });
    case types.COMMENTS_UPDATE:
      return _extends({}, state, {
        [payload.id]: _extends({}, state[payload.id], payload.comment)
      });
    case types.COMMENTS_LIKE_SET:
      return _extends({}, state, {
        [payload.id]: _extends({}, state[payload.id], {
          likers: payload.newVal ? [...state[payload.id].likers, payload.user] : state[payload.id].likers.filter(u => u !== payload.user)
        })
      });
    default:
      return state;
