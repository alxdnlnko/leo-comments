...
function* commentsChanged(change) {
  if (change.event === 'child_added') yield put(actions.addComment(change.payload.id, change.payload));else yield put(actions.updateComment(change.payload.id, change.payload));
}
